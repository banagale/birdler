...
# Birdler run script: generates the Style poem with the Ripley voice
# Usage: ./run_ripley_style.sh
...
echo "[run] Option A — expressive, deterministic, larger chunks, parallel"
poetry run python birdler.py \
  --voice "$VOICE" \
  --text-file "$TEXT_FILE" \
  --output-dir "$OUT_DIR" \
...
echo "[run] Option B — Option A + post-processing (denoise, auto-editor, EBU normalize)"
poetry run python birdler.py \
  --voice "$VOICE" \
  --text-file "$TEXT_FILE" \
  --output-dir "$OUT_DIR" \
...
echo "[done] Runs completed. Check $OUT_DIR and adjacent .settings.json/.csv files."
